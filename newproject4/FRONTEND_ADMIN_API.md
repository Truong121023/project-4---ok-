...
- Each section still contains only `title`, `content`, and optional `imagePath`; there is still no section `id` in request or response.
- Upload images first through `/api/admin/uploads/images`, then place the returned path into `sections[].imagePath`.
...
5. If the role is `MANAGER`, keep the UI limited to review moderation only.
...
- `MANAGER` access is intentionally narrower than `ADMIN` access.
