  ...
  return (
    <div className="rounded-xl border border-ink-900/10 bg-cream-50 p-5 shadow-soft">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className={ui.eyebrow}>Delivery proof</p>
          <p className="mt-1 text-sm leading-6 text-ink-600">
            Upload delivery proof. The backend stores it on the order and
            automatically moves the order to Completed.
          </p>
        </div>
        {order.deliveryProofUploadedAt ? (
          <span className={ui.pill}>
            Uploaded {formatDateTimeVn(order.deliveryProofUploadedAt)}
          </span>
        ) : null}
      </div>
        ...
        <div className="rounded-lg border border-ink-900/10 bg-cream-100 p-4">
          {deliveryProofPreviewUrl ? (
            ...
            <div className="grid h-64 place-items-center rounded-lg border border-dashed border-ink-900/15 bg-beige-100 px-4 text-center text-sm leading-7 text-ink-400">
              No delivery proof image uploaded yet.
            </div>
          )}
          ...
          {order.deliveryProofCapturedAt ? (
            <p className="mt-2 text-xs text-ink-400">
              Captured: {formatDateTimeVn(order.deliveryProofCapturedAt)}
            </p>
          ) : null}
        </div>
        ...
        <form className="grid gap-3" onSubmit={onProofUpload}>
          ...
          <label className="grid gap-1.5">
            ...
            <textarea
              ...
              placeholder="e.g. delivered in tower B lobby, customer confirmed receipt."
          ...
          <div>
            <button
              ...
              type="submit"
            >
              {proofForm.uploading ? "Uploading…" : "Upload proof image"}
            </button>
          </div>
        </form>
      </div>
