...
function mapPromotionCard(promotion) {
  return {
    ...
    availableRedemptions: toNumber(promotion?.availableRedemptions, 0),
  };
}
...
function mapUserNotification(notification) {
  ...
  return {
    ...
    title: translateUiText(pickText(notification?.title)),
    message: translateUiText(pickText(
...
function mapUserCurrentLevel(level) {
  return {
    ...
    creditPoints: toNumber(level?.creditPoints, 0),
    levelMinCreditPoints: toNullableNumber(level?.levelMinCreditPoints),
    nextLevelId: toId(level?.nextLevelId),
    nextLevelCode: pickText(level?.nextLevelCode),
    nextLevelName: pickText(level?.nextLevelName),
    nextLevelMinCreditPoints: toNullableNumber(level?.nextLevelMinCreditPoints),
    membershipPoints: toNumber(level?.membershipPoints, 0),
    levelMinMembershipPoints: toNumber(level?.levelMinMembershipPoints, 0),
    nextLevelMinMembershipPoints: toNumber(level?.nextLevelMinMembershipPoints, 0),
  };
}

function mapUserLevelDefinition(level) {
  const thresholdPoints = toNumber(
    pickValue(level?.minMembershipPoints, level?.minPaidAmount, level?.minCreditPoints),
    0,
  );
  ...
  return {
    ...
    storeSlug: pickText(level?.storeSlug, level?.slug),
    storeName: pickText(level?.storeName),
    code: pickText(level?.code),
    name: pickText(level?.name),
    minPaidAmount: toNumber(level?.minPaidAmount, 0),
    minCreditPoints: toNullableNumber(level?.minCreditPoints),
    minMembershipPoints: thresholdPoints,
    active: Boolean(pickValue(level?.active, true)),
    createdAt: pickText(level?.createdAt),
    updatedAt: pickText(level?.updatedAt),
  };
}
...
function mapCheckoutPricing(payload) {
  return {
    ...
    statusSummary: translateUiText(pickText(payload?.statusSummary)),
  };
}
...
function mapOrder(payload, includeNestedOrders = true) {
  ...
  return {
    ...
    creditPointsAwarded: toNumber(payload?.creditPointsAwarded, 0),
    promotionCode: pickText(payload?.promotionCode),
...
function mapEmployeeOrderScanResponse(payload) {
  return {
    ...
    message: translateUiText(pickText(payload?.message)),
...
function mapPublicHomePayload(payload) {
  return {
    ...
    promotions: ensureArray(payload?.promotions).map(mapPromotionCard),
...
export async function fetchUserLevelDefinitions(auth) {
  const payload = await apiRequest("/api/user/levels/definitions", authOptions(auth));
  const items = ensureArray(payload);
  return items.map(mapUserLevelDefinition);
}

export async function fetchAdminUserLevelDefinitions(auth) {
  const payload = await apiRequest("/api/admin/user-levels", authOptions(auth));
  const items = ensureArray(payload);
  return items.map(mapUserLevelDefinition);
}

function buildAdminUserLevelDefinitionPayload(input) {
  const thresholdPoints = toNullableNumber(input?.minMembershipPoints);
  const normalizedStoreId = String(input?.storeId ?? "").trim();

  return {
    storeId: normalizedStoreId ? toRequestNumber(normalizedStoreId) : null,
    code: pickText(input?.code).trim().toUpperCase(),
    name: pickText(input?.name).trim(),
    minPaidAmount: thresholdPoints,
    minMembershipPoints: thresholdPoints,
    active: Boolean(pickValue(input?.active, true)),
  };
}

export async function createAdminUserLevelDefinition(auth, input) {
  const payload = await apiRequest("/api/admin/user-levels", {
    method: "POST",
    body: buildAdminUserLevelDefinitionPayload(input),
    ...authOptions(auth),
  });

  return mapUserLevelDefinition(payload);
}

export async function updateAdminUserLevelDefinition(auth, definitionId, input) {
  const payload = await apiRequest(`/api/admin/user-levels/${definitionId}`, {
    method: "PUT",
    body: buildAdminUserLevelDefinitionPayload(input),
    ...authOptions(auth),
  });
...
export async function deleteAdminUserLevelDefinition(auth, definitionId) {
  return apiRequest(`/api/admin/user-levels/${definitionId}`, {
    method: "DELETE",
    ...authOptions(auth),
  });
}

export async function fetchUserVoucherCatalog(auth) {
  const payload = await apiRequest("/api/user/vouchers", authOptions(auth));
  const items = ensureArray(payload);
  return items.map(mapPromotionCard);
}

export async function redeemUserVoucher(auth, promotionId) {
  const payload = await apiRequest(`/api/user/vouchers/${promotionId}/redeem`, {
    method: "POST",
    ...authOptions(auth),
  });
  ...
  return {
    ...
    promotionId: toId(payload?.promotionId),
    promotionCode: pickText(payload?.promotionCode),
    remainingCreditPoints: toNumber(payload?.remainingCreditPoints, 0),
    availableRedemptions: toNumber(payload?.availableRedemptions, 0),
  };
}
...
export async function createUserDeliveryAddress(auth, payload) {
  const response = await apiRequest("/api/user/delivery-addresses", {
    ...
    body: {
      ...
      latitude: toRequestNumber(payload?.latitude),
      longitude: toRequestNumber(payload?.longitude),
