...
const AUTH_STORAGE_KEY = "kamatcha.auth";
...
export function getApiErrorMessage(error, fallback = "Something went wrong.") {
  if (error instanceof ApiError) {
    ...
    if (validationMessage) {
      if (error.message && error.message !== "Validation failed") {
        return translateUiText(`${error.message}: ${validationMessage}`);
      }

      return translateUiText(validationMessage);
    }

    return translateUiText(error.message);
  }
  ...
  return translateUiText(fallback);
}
